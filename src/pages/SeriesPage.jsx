...
function SeriesPage() {
  ...
  if (searchResults.length > 0) {
    return (
      <main className="series-page mx-4 mt-6 md:mt-8 lg:pl-[124px] md:mx-0  pb-[13px]">
        <SearchBar />
  ...
  return (
    <main className="series-page mx-4 mt-6 md:mt-8 lg:pl-[124px] md:mx-0  pb-[13px]">
      <SearchBar />
      <section aria-labelledby="movies-heading" className="my-6">
        <h2 id="movies-heading" className="mb-6 md:text-[2rem]">
          TV Series
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-[30px]">
          {allMovies
            .filter((allmovie) => allmovie.category === "TV Series")
            .map((movie, index) => {
              return (
                <Thumbnail
                  key={index}
